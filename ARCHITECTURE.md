...
The MVP is a standalone enterprise clinical assistant that connects to OpenEMR as the source of truth for authentication and patient data. It is not a PHP-native OpenEMR module for the MVP. The user experience is a Next.js chat application where an authenticated clinician selects one authorized patient, asks questions about that patient's chart, and receives read-only answers with citations.

The backend is a Python FastAPI service. It validates OpenEMR-issued identity, enforces role and patient access, retrieves evidence through deterministic clinical data tools and vector search, calls a model through a provider adapter, verifies factual claims against evidence, and streams status plus the final answer back to the browser.
